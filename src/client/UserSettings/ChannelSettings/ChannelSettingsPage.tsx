...
import { ChannelPanels } from "miracle-tv-client/UserSettings/ChannelSettings/ChannelPanels";
...
const tabs = (tFn?: TranslationFn) =>
  reject(equals(""))({
    ...
    panels: publicRuntimeConfig ? tFn("ui-tabs-panels") ?? "Panels" : null,
  });
...
export const ChannelSettingsPage = ({
  ...
  const formValues: Partial<UpdateChannelInput> = useMemo(
    () => ({
      id: channel?.id,
      panels: channel?.panels?.map(({ id }) => id) ?? [],
    }),
    [channel]
  );
  ...
  return (
    <Form<Partial<UpdateChannelInput>>
      onSubmit={() => {}}
              ...
              {tabList.map((tab) => (
                ...
                >
                  <Link
                    ...
                    title={
                      !canViewKeys
                        ? tChannel("stream-keys-forbidden")
                        : undefined
                    }
                    py={2}
                    px={3}
