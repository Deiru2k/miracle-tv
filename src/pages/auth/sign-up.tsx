...
const SignUpPage = () => {
  ...
  const [signUpMutation, { loading: isSignInLoading }] = useSignUpMutation({
    onCompleted: ({ signUp: { id, username } }) => {
      toast({ status: "success", title: `Welcome, ${username}!\nYou may now login with your credentials.`})
      push("/auth/login");
    },
    onError: (e) => {
  ...
  const signUp = useCallback((values: { username: string, password: string, email: string }) => {
        ...
        <Form onSubmit={signUp}>
          {({ handleSubmit }) => (
            <form onSubmit={handleSubmit}>
              <FormInput name="username" mb={4} isRequired label="Username" hideLabel />
              <FormInput name="password" type="password" mb={4} isRequired label="Password" hideLabel />
              <FormInput name="email" type="email" mb={4} isRequired label="E-mail" hideLabel />
              <Button w="100%" type={"submit"}>Sign Up</Button>
              <Box width="100%" mt={4}>
